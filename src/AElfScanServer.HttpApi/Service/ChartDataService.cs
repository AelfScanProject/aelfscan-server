...
public class ChartDataService : AbpRedisCache, IChartDataService, ITransientDependency
{
    ...
    public async Task<DailyTransactionFeeResp> GetMergeDailyTransactionFeeRespAsync()
    {
        ...
        {
            var curTotalFeeElf = (double.Parse(dailyAvgTransactionFee.TotalFeeElf) / 1e8).ToString("F6");
            ...
            {
                dailyAvgTransactionFee.SideChainTotalFeeElf = (double.Parse(v.TotalFeeElf) / 1e8).ToString("F6");
                dailyAvgTransactionFee.MergeTotalFeeElf = (double.Parse(dailyAvgTransactionFee.MergeTotalFeeElf) +
                                                           double.Parse(v.TotalFeeElf) / 1e8)
                    .ToString("F6");
            }
        }
        ...
        var resp = new DailyTransactionFeeResp()
        {
            List = mainDataList.ToList(),
            Total = mainDataList.Count,
            Highest = mainDataList.MaxBy(c => double.Parse(c.MergeTotalFeeElf)),
            Lowest = mainDataList.MinBy(c => double.Parse(c.MergeTotalFeeElf)),
        };
        ...
        foreach (var dailyAvgTransactionFee in mainDataList)
        {
            ...
            if (dic.TryGetValue(dailyAvgTransactionFee.Date, out var v))
            {
                dailyAvgTransactionFee.MergeAvgFeeUsdt =
                    ((double.Parse(dailyAvgTransactionFee.MergeAvgFeeUsdt) * dailyAvgTransactionFee.TransactionCount +
                      double.Parse(v.AvgFeeUsdt) * v.TransactionCount / 1e8) /
    ...
    public async Task<DailyTotalBurntResp> GetMergeDailyTotalBurntRespAsync()
    {
        ...
        foreach (var data in mainDataList)
        {
            var burntDouble = double.Parse(data.Burnt);
            data.Burnt = double.Parse(data.Burnt).ToString("F6");
            data.MainChainBurnt = double.Parse(data.Burnt).ToString("F6");
            data.MergeBurnt = double.Parse(data.Burnt).ToString("F6");
            if (dic.TryGetValue(data.DateStr, out var sideData))
            {
                var sideBurnt = double.Parse(sideData.Burnt);
        ...
        var resp = new DailyTotalBurntResp()
        {
            List = mainDataList.ToList(),
            Total = mainDataList.Count,
            Highest = mainDataList.MaxBy(c => double.Parse(c.MergeBurnt)),
            Lowest = mainDataList.MinBy(c => double.Parse(c.MergeBurnt)),
        };
